...
% Gera JSONs em data/proofs/ para cada dogma definido.
...
main :-
    ...
    make_directory_path('data/proofs'),
    ...
    setup_call_cleanup(open('data/proofs/index.json', write, SI), write(SI, IndexJson), close(SI)),
    halt.

export_one(Id) :-
    format('Exporting ~w~n',[Id]),
    atomic_list_concat(['data/proofs/', Id, '.json'], Path),
